...
@Component({
  selector: 'app-patient-pav',
  templateUrl: './patient-pav.component.html',
  styleUrls: ['./patient-pav.component.css']
})
export class PatientPavComponent implements OnInit{
  formS: FormGroup;
  pervValue: any;
  ...
  delPavInj(index: number) {
    let e = this.subs.at(index);
    this.patientService.delPavInj(e.get('drugId').value).subscribe();
  ...
  async createPavInj() {
    ...
    if(this.formS.controls['newDrugName'].valid && this.formS.controls['newYnName'].valid){
      ...
      let drugId  =  await firstValueFrom(this.patientService.createPavInj(this.patientId, ynName, drugName, dateStart, dateEnd))
    ...
    if(!(JSON.stringify(oldValue) === JSON.stringify(curValue)))
      for (let index = 0; index < oldValue.length; index++) {
        if(!(JSON.stringify(oldValue[index]) === JSON.stringify(curValue[index]))){
          this.patientService.updatePavInj
          (
